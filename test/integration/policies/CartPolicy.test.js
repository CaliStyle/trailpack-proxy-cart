...
describe('CartPolicy', () => {
  ...
  it('should make checkout post request', (done) => {
    agent
      ...
      .end((err, res) => {
        ...
        assert.equal(res.body.order.order_items[0].product_id, shopProducts[4].id)
        assert.equal(res.body.order.order_items[0].price, shopProducts[4].price)
        assert.equal(res.body.order.order_items[0].total_discounts, 200)
        assert.equal(res.body.order.order_items[0].calculated_price, shopProducts[4].price - 200)
        assert.equal(res.body.order.order_items[0].product_id, shopProducts[4].id)
        ...
        assert.equal(res.body.order.order_items[1].product_id, shopProducts[3].id)
        assert.equal(res.body.order.order_items[1].price, shopProducts[3].price)
        assert.equal(res.body.order.order_items[1].total_discounts, 100)
        assert.equal(res.body.order.order_items[1].calculated_price, shopProducts[3].price - 100)
        assert.equal(res.body.order.order_items[1].product_id, shopProducts[3].id)
  ...
  it('should renew subscription', done => {
    global.app.services.SubscriptionService.renew(subscriptionID)
      .then(body => {
        ...
        assert.equal(body.order.order_items[0].product_id, shopProducts[4].id)
        assert.equal(body.order.order_items[0].price, shopProducts[4].price)
        assert.equal(body.order.order_items[0].total_discounts, 200)
        assert.equal(body.order.order_items[0].calculated_price, shopProducts[4].price - 200)
        assert.equal(body.order.order_items[0].product_id, shopProducts[4].id)
        ...
        assert.equal(body.order.order_items[1].product_id, shopProducts[3].id)
        assert.equal(body.order.order_items[1].price, shopProducts[3].price)
        assert.equal(body.order.order_items[1].total_discounts, 100)
        assert.equal(body.order.order_items[1].calculated_price, shopProducts[3].price - 100)
        assert.equal(body.order.order_items[1].product_id, shopProducts[3].id)
  ...
  it('should deactivate customer subscription by id', done => {
    agent
      .post(`/subscription/${ subscriptionID }/deactivate`)
      .send({})
      .expect(200)
      .end((err, res) => {
        // console.log('THIS POLICY subscription', res.body)
