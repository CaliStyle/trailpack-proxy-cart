...
describe('Admin User Manual Fulfillment Physical', () => {
  ...
  it('should create a cart with shipping address for manual fulfillment', (done) => {
    adminUser
      .post('/cart')
      .send({
        shipping_address: {
  ...
  it('should add physical product to cart that has manual fulfilment', done => {
    adminUser
      .post('/cart/addItems')
      .send({
        line_items: [
          {
            product_id: shopProducts[14].id
          }
        ]
      })
      .expect(200)
      .end((err, res) => {
        ...
        assert.equal(res.body.total_due, shopProducts[14].price)
        done(err)
      })
  })
  ...
  it('should checkout and item should be pending fulfillment', (done) => {
    adminUser
      ...
      .end((err, res) => {
        ...
        // Pricing
        assert.equal(res.body.order.total_line_items_price, shopProducts[14].price)
        assert.equal(res.body.order.subtotal_price, shopProducts[14].price)
        assert.equal(res.body.order.total_price, shopProducts[14].price)
        ...
        assert.equal(res.body.order.order_items[0].price, shopProducts[14].price)
        assert.equal(res.body.order.order_items[0].price_per_unit, shopProducts[14].price)
        assert.equal(res.body.order.order_items[0].calculated_price, shopProducts[14].price)
        ...
        assert.equal(res.body.order.total_captured, shopProducts[14].price)
  ...
  it('should set pending manual to sent by fulfillment id', (done) => {
    ...
    adminUser
      ...
      .end((err, res) => {
        ...
        assert.equal(res.body.total_sent_fulfillments, 1)
